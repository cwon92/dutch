...
@Getter
@Setter
public class DutchRegisterPagingCreatorDTO {

	private DutchRegisterPagingDTO dutchRegisterPaging ; 
	...
	public DutchRegisterPagingCreatorDTO(long rowTotal, 
			                     DutchRegisterPagingDTO dutchRegisterPaging, 
			                     List<MemberVO> memberList) {


		this.rowTotal = rowTotal ;
		this.dutchRegisterPaging = dutchRegisterPaging ;
		this.memberList = memberList ;
		this.pagingNumCnt = 3 ;	

		//계산된 끝 페이징 번호:
		this.endPagingNum = (int) Math.ceil((double) dutchRegisterPaging.getPageNum()/this.pagingNumCnt) * this.pagingNumCnt;

		//계산된 시작 페이징 번호:
		this.startPagingNum = this.endPagingNum - (this.pagingNumCnt -1) ;

		//총 페이지 수 = 맨 마지막 페이지번호
		this.lastPageNum = (int) Math.ceil((double)this.rowTotal / this.dutchRegisterPaging.getRowAmountPerPage());
