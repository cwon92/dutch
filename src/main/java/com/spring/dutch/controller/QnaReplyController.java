...
//	@PreAuthorize("isAuthenticated()")
	public ResponseEntity<String> registerReplyForQna(@PathVariable("qno") long qno,
		...
		Long registeredQno = qnareplyService.registerReplyForQna(qnareply);
		String _registeredQno = null;
		
		if(registeredQno != null) {
			_registeredQno = String.valueOf(registeredQno);
		}else {
			_registeredQno = String.valueOf(registeredQno);
		}
		
		return registeredQno != null ? new ResponseEntity<String>(_registeredQno, HttpStatus.OK)
									 : new ResponseEntity<String>(_registeredQno, HttpStatus.INTERNAL_SERVER_ERROR) ;
	}
...
//	@PreAuthorize("isAuthenticated() && principal.username == #qnareply.mno")
	...
	//특정 게시물에 대한 특정 댓글/답글 삭제(qrdelFalg를 1로 업뎃)
